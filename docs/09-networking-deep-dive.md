...
- Understand how DNS, Services, kube-proxy, and packet flow behave across Linux and Windows nodes.
...
This module focuses on how DNS, Services, kube-proxy, and packet flow behave across Linux and Windows nodes. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains how to trace traffic from client pod to backend pod so abstractions remain grounded in packet flow and dataplane behavior. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- Observability and packet debugging commands differ across Linux and Windows, even when the logical service model is the same
...
```powershell
# TODO: add Windows-specific commands for networking deep dive.
...
```bash
# TODO: add Linux-specific commands for networking deep dive.
...
```yaml
# TODO: add Kubernetes manifests or snippets for networking deep dive.
...
- Capture known-good traffic flows and service checks so later incidents have a baseline.
...
- Kubernetes documentation relevant to networking deep dive
...
flowchart LR
    ...
    ControlPlane --> Module[Networking Deep Dive]
    LinuxWorker --> Module
