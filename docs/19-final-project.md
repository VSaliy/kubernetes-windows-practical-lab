...
- Understand how to combine Linux frontend, Java services, Windows services, storage, observability, and identity into one architecture.
...
This module focuses on how to combine Linux frontend, Java services, Windows services, storage, observability, and identity into one architecture. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains why the final exercise should function as an architecture review and operations exercise rather than just a deployment demo. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- Windows-specific dependencies should stay visible in the final architecture instead of disappearing behind generic microservice terminology
...
```powershell
# TODO: add Windows-specific commands for final project.
...
```bash
# TODO: add Linux-specific commands for final project.
...
```yaml
# TODO: add Kubernetes manifests or snippets for final project.
...
- Score the final project on clarity, operability, and failure readiness, not only on successful deployment.
...
- Kubernetes documentation relevant to final project
...
flowchart LR
    ...
    ControlPlane --> Module[Final Project]
    LinuxWorker --> Module
