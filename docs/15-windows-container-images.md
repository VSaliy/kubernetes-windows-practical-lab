...
- Understand how base image choice, compatibility, layering, and patch alignment affect Windows workloads.
...
This module focuses on how base image choice, compatibility, layering, and patch alignment affect Windows workloads. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains why Windows image lifecycle management needs to be treated as a platform constraint, not a packaging detail. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- Windows host and image versions must align closely, unlike many Linux container workflows
...
```powershell
# TODO: add Windows-specific commands for windows container images.
...
```bash
# TODO: add Linux-specific commands for windows container images.
...
```yaml
# TODO: add Kubernetes manifests or snippets for windows container images.
...
- Maintain an approved image catalog linked to the active Windows node baselines.
...
- Kubernetes documentation relevant to windows container images
...
flowchart LR
    ...
    ControlPlane --> Module[Windows Container Images]
    LinuxWorker --> Module
