...
- Understand deploying baseline Linux applications and diagnostics pods with explicit Linux scheduling.
...
This module focuses on deploying baseline Linux applications and diagnostics pods with explicit Linux scheduling. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains why simple Linux workloads are valuable for validating nodes, Services, probes, and base network behavior before more complex workloads arrive. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- Linux workloads have a broader tooling and image ecosystem, which makes them useful as a baseline when comparing Windows behavior
...
```powershell
# TODO: add Windows-specific commands for linux workloads.
...
```bash
# TODO: add Linux-specific commands for linux workloads.
...
```yaml
# TODO: add Kubernetes manifests or snippets for linux workloads.
...
- Use lightweight Linux diagnostics workloads as permanent smoke tests for platform health.
...
- Kubernetes documentation relevant to linux workloads
...
flowchart LR
    ...
    ControlPlane --> Module[Linux Workloads]
    LinuxWorker --> Module
