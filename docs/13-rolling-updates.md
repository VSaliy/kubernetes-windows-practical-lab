...
- Understand how deployments, readiness, draining, and rollback behavior differ across Linux and Windows workloads.
...
This module focuses on how deployments, readiness, draining, and rollback behavior differ across Linux and Windows workloads. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains why rollout mechanics must be understood at the workload and node-capacity level instead of treated as a single kubectl command. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- Windows images are larger and startup times can be slower, which changes rollout and rollback expectations
...
```powershell
# TODO: add Windows-specific commands for rolling updates.
...
```bash
# TODO: add Linux-specific commands for rolling updates.
...
```yaml
# TODO: add Kubernetes manifests or snippets for rolling updates.
...
- Test surge, unavailable, and drain strategy separately for Linux and Windows application tiers.
...
- Kubernetes documentation relevant to rolling updates
...
flowchart LR
    ...
    ControlPlane --> Module[Rolling Updates]
    LinuxWorker --> Module
