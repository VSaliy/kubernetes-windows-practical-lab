...
- Understand how host hardening, RBAC, image trust, secrets handling, and workload isolation fit the lab.
...
This module focuses on how host hardening, RBAC, image trust, secrets handling, and workload isolation fit the lab. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains why platform security spans the Windows host, Linux control plane, runtimes, identities, workloads, and operational process. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- Windows security baselines often intersect with Active Directory, gMSA, and enterprise endpoint controls
...
```powershell
# TODO: add Windows-specific commands for security.
...
```bash
# TODO: add Linux-specific commands for security.
...
```yaml
# TODO: add Kubernetes manifests or snippets for security.
...
- Separate learning-lab shortcuts from production-required controls in every module.
...
- Kubernetes documentation relevant to security
