...
- Understand how persistent data patterns, CSI integrations, and SMB-backed options fit a hybrid cluster.
...
This module focuses on how persistent data patterns, CSI integrations, and SMB-backed options fit a hybrid cluster. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains why storage must be designed around filesystem semantics, access modes, node compatibility, and failure recovery. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- Windows workloads often surface SMB integration and filesystem semantics earlier than Linux-only clusters
...
```powershell
# TODO: add Windows-specific commands for storage.
...
```bash
# TODO: add Linux-specific commands for storage.
...
```yaml
# TODO: add Kubernetes manifests or snippets for storage.
...
- Document backup, restore, and failover behavior before declaring any storage pattern production-ready.
...
- Kubernetes documentation relevant to storage
