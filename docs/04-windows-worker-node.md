...
- Understand how a Windows Server 2022 node joins the cluster and what it takes to keep it compatible with the rest of the platform.
...
This module focuses on how a Windows Server 2022 node joins the cluster and what it takes to keep it compatible with the rest of the platform. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains why Windows worker support is different in image lifecycle, runtime behavior, networking integration, and troubleshooting workflow. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- Windows supports worker nodes only, and host/image version alignment matters far more than in most Linux container workflows
...
```powershell
# TODO: add Windows-specific commands for windows worker node.
...
```bash
# TODO: add Linux-specific commands for windows worker node.
...
```yaml
# TODO: add Kubernetes manifests or snippets for windows worker node.
...
- Keep Windows node patch baselines and image tags aligned deliberately to avoid unpredictable scheduling failures.
...
- Kubernetes documentation relevant to windows worker node
...
flowchart LR
    ...
    ControlPlane --> Module[Windows Worker Node]
    LinuxWorker --> Module
