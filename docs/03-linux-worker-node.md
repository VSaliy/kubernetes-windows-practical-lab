...
- Understand how Linux workers join the cluster and provide the broadest workload and ecosystem compatibility.
...
This module focuses on how Linux workers join the cluster and provide the broadest workload and ecosystem compatibility. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains how kubelet, containerd, CNI, and kube-proxy combine to make a schedulable Linux node available to the cluster. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- Linux worker nodes typically host the widest set of add-ons and operational tooling compared with Windows workers
...
```powershell
# TODO: add Windows-specific commands for linux worker node.
...
```bash
# TODO: add Linux-specific commands for linux worker node.
...
```yaml
# TODO: add Kubernetes manifests or snippets for linux worker node.
...
- Reserve Linux worker capacity for cluster add-ons and diagnostics even if the business workloads are mixed.
...
- Kubernetes documentation relevant to linux worker node
...
flowchart LR
    ...
    ControlPlane --> Module[Linux Worker Node]
    LinuxWorker --> Module
