...
- Understand deploying IIS-style Windows workloads and diagnostics pods onto Windows nodes.
...
```powershell
# TODO: add Windows-specific commands for windows workloads.
...
```bash
# TODO: add Linux-specific commands for windows workloads.
...
```yaml
# TODO: add Kubernetes manifests or snippets for windows workloads.
...
- Standardize a small set of approved Windows base images and test them against the active node patch level.
...
- Kubernetes documentation relevant to windows workloads
...
flowchart LR
    ...
    ControlPlane --> Module[Windows Workloads]
    LinuxWorker --> Module
