# Linux Control Plane with kubeadm

## Learning objectives

- Understand the bootstrap path for certificates, static pods, etcd, kubelet, and the API server on Ubuntu Server.
...
This module focuses on the bootstrap path for certificates, static pods, etcd, kubelet, and the API server on Ubuntu Server. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains why kubeadm is useful for learning because it exposes the control-plane assembly process instead of hiding it behind a managed installer. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- The control plane must stay on Linux because core Kubernetes control-plane components and lifecycle assumptions remain Linux-based
...
```powershell
# TODO: add Windows-specific commands for linux control plane with kubeadm.
...
```bash
# TODO: add Linux-specific commands for linux control plane with kubeadm.
...
```yaml
# TODO: add Kubernetes manifests or snippets for linux control plane with kubeadm.
...
- Version, backup, and restore decisions for the control plane should be documented as architecture, not just shell history.
...
- Kubernetes documentation relevant to linux control plane with kubeadm
...
flowchart LR
    ...
    ControlPlane --> Module[Linux Control Plane with kubeadm]
    LinuxWorker --> Module
