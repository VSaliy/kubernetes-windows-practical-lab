...
- Understand the host, VM, node, and workload topology that the rest of the lab builds on.
...
This module focuses on the host, VM, node, and workload topology that the rest of the lab builds on. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains why Linux control plane plus Windows worker architecture is required, what the trust boundaries are, and how node roles map to platform capabilities. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- Windows worker nodes cannot host Kubernetes control-plane components and often require separate operational baselines, image compatibility checks, and network validation steps
...
```powershell
# TODO: add Windows-specific commands for architecture baseline.
...
```bash
# TODO: add Linux-specific commands for architecture baseline.
...
```yaml
# TODO: add Kubernetes manifests or snippets for architecture baseline.
...
- Treat the architecture document as the contract for every later module and keep Linux-only versus Windows-capable responsibilities explicit.
...
- Kubernetes documentation relevant to architecture baseline
...
flowchart LR
    ...
    ControlPlane --> Module[Architecture Baseline]
    LinuxWorker --> Module
