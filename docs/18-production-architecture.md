...
- Understand how the lab evolves into a more durable architecture with higher availability and stronger controls.
...
This module focuses on how the lab evolves into a more durable architecture with higher availability and stronger controls. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains which shortcuts are acceptable in a learning lab and which must be redesigned for production use. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- Production Windows support raises extra questions around patch sequencing, compatibility testing, and support boundaries
...
```powershell
# TODO: add Windows-specific commands for production architecture.
...
```bash
# TODO: add Linux-specific commands for production architecture.
...
```yaml
# TODO: add Kubernetes manifests or snippets for production architecture.
...
- Make every production recommendation traceable to a risk or limitation observed in the lab.
...
- Kubernetes documentation relevant to production architecture
...
flowchart LR
    ...
    ControlPlane --> Module[Production Architecture]
    LinuxWorker --> Module
