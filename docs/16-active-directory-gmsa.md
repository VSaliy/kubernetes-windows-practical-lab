# Active Directory and gMSA

## Learning objectives

- Understand how enterprise identity flows into Windows workloads through gMSA and related components.
...
This module focuses on how enterprise identity flows into Windows workloads through gMSA and related components. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains why directory integration changes network, DNS, security, and workload design in hybrid enterprise environments. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- gMSA is specific to Windows-integrated workloads and has no direct Linux equivalent
...
```powershell
# TODO: add Windows-specific commands for active directory and gmsa.
...
```bash
# TODO: add Linux-specific commands for active directory and gmsa.
...
```yaml
# TODO: add Kubernetes manifests or snippets for active directory and gmsa.
...
- Prototype identity integration early because directory dependencies shape the rest of the platform.
...
- Kubernetes documentation relevant to active directory and gmsa
...
flowchart LR
    ...
    ControlPlane --> Module[Active Directory and gMSA]
    LinuxWorker --> Module
