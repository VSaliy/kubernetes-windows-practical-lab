...
- Understand how virtual switching, IP allocation, routing, and DNS are designed before any node is bootstrapped.
...
This module focuses on how virtual switching, IP allocation, routing, and DNS are designed before any node is bootstrapped. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains why Hyper-V network choices directly affect kubelet registration, pod networking, service routing, and troubleshooting clarity. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- Hyper-V switch design, NAT behavior, and host adapter selection are Windows host concerns that do not exist in the same form on Linux hypervisors
...
```powershell
# TODO: add Windows-specific commands for hyper-v networking.
...
```bash
# TODO: add Linux-specific commands for hyper-v networking.
...
```yaml
# TODO: add Kubernetes manifests or snippets for hyper-v networking.
...
- Use stable addressing, documented DNS, and transparent routing so cluster failures can be analyzed from first principles.
...
- Kubernetes documentation relevant to hyper-v networking
...
flowchart LR
    ...
    ControlPlane --> Module[Hyper-V Networking]
    LinuxWorker --> Module
