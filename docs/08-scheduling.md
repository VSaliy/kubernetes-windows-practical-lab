...
- Understand how labels, selectors, taints, tolerations, and affinity keep workloads on the correct operating system and node role.
...
This module focuses on how labels, selectors, taints, tolerations, and affinity keep workloads on the correct operating system and node role. The recommended baseline keeps the Windows 11 host responsible for Hyper-V and operator tooling, Ubuntu Server nodes responsible for the Kubernetes control plane and Linux worker capacity, and Windows Server 2022 nodes responsible for Windows workload execution. Each step should reinforce those boundaries rather than blur them.

## Theory

This module explains why scheduling should be treated as an intentional placement policy rather than a side effect of default behavior. The goal is to make the moving parts visible enough that you can reason about normal behavior, failure behavior, and operational trade-offs without relying on a black-box installer.

## Windows-specific differences

- Mixed-OS clusters fail quickly when manifests omit OS-aware selectors or rely on vague node intent
...
```powershell
# TODO: add Windows-specific commands for scheduling and placement.
...
```bash
# TODO: add Linux-specific commands for scheduling and placement.
...
```yaml
# TODO: add Kubernetes manifests or snippets for scheduling and placement.
...
- Encode platform intent in policy and labels so safe scheduling does not depend on operator memory.
...
- Kubernetes documentation relevant to scheduling and placement
...
flowchart LR
    ...
    ControlPlane --> Module[Scheduling and Placement]
    LinuxWorker --> Module
